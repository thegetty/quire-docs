...
If you are including PDF and e-book downloads as part of your online site (this is Quire’s default) you’ll need to update those files by running `quire epub` and `quire pdf` as described below **before** running `quire build`.

Links to download the EPUB, MOBI, and PDF versions are automatically included in the sidebar menu of the online version of your project. These links can be removed or modified by making changes to the `resource_link` information in your `content/_data/publication.yaml` file. You can add links to these files from *anywhere* in your markdown files by linking to `_assets/downloads/output.epub`, `_assets/downloads/output.mobi`, and `_assets/downloads/output.pdf` respectively.
...
To include the EPUB file as a download from your online edition, mvove the `epubjs.epub` file into `content/_assets/downloads/` directory. Be sure to update the the `resource_link` information in your `content/_data/publication.yaml` file as described above.
...
1. Open the `config.yaml` file and at the bottom of the list of `params` temporarily add `pdf: true`, which tells Quire to build the PDF version of your site. Be sure to delete this line when you’re done working on the PDF.
