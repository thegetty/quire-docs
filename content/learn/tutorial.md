...
Once installed, open your text editor, navigate to File, and open the `my-project` directory you created in Step 3 above. The `my-project` directory also lies in your {{< q-def "home directory">}}. You should see all of the directory’s contents listed.
...
2. Leave the `type` as `"page"`. [Other page types available](/documentation/pages/#define-page-types) are `"essay"`, `"entry"`, `"cover"`, and `"contents"`. Each displays the page content and data differently. The default is `"page"`.
